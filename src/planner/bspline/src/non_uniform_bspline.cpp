...
bool NonUniformBspline::getTimeSpan(double& um, double& um_p) {
...
bool NonUniformBspline::reallocateTime_mod(bool show) // order = 3!
{
  // SETY << "[Bspline]: total points size: " << control_points_.rows() << endl;
  // cout << "origin knots:\n" << u_.transpose() << endl;
  bool fea = true;
  ...
  for (int i = 3; i < P.rows() - 1; ++i) {
    ...
    if (fabs(vel(0)) > limit_vel_ + 1e-4 || fabs(vel(1)) > limit_vel_ + 1e-4 ||
      ...
      int knot_start;
      if ( i + 2 > 5 ){ 
        t_inc = delta_t / double(p_);
        knot_start = i+2;
      }
      else{ // To avoid affecting start constraints
        t_inc = delta_t / double(i + p_ + 1 - 5);
        knot_start = 6;
      }
      for (int j = knot_start; j <= i + p_ + 1; ++j) {
        u_(j) += double(j - i - 1) * t_inc;
        if (j <= 5 && j >= 1) {
          cout << "\033[31mvel j: " << j << "\033[0m" << endl;
        }
      }
  ...
  for (int i = 1; i < P.rows() - 2; ++i) {
    ...
    if (fabs(acc(0)) > limit_acc_ + 1e-4 || fabs(acc(1)) > limit_acc_ + 1e-4 ||
      ...
      //     if (j <= 5 && j >= 1) {
      //       cout << "\033[31macc j: " << j << "\033[0m" << endl;
      //     }
      //   }
      ...
      if (i == 1) 
      {  
        double ratio = max_acc / limit_acc_ + 1e-4;
        if (ratio > limit_ratio_) ratio = limit_ratio_;
        // cout << "ratio: " << ratio << endl;

        double time_ori = u_(6) - u_(5);
        double k = 3*ratio*(P(i+2,infes_dime)-P(i+1,infes_dime))/((P(i+2,infes_dime)-P(i+1,infes_dime))+(ratio-1)*(P(i+1,infes_dime)-P(i,infes_dime)))-2;
        double time_new = k * time_ori;
        double delta_t = time_new - time_ori;

        cout << "i=1" << " time_ori=" << time_ori << " k=" << k << " time_new=" << time_new << " delta_t=" << delta_t << endl;
      ...
      else if (i == 2)
      {
        ...
        double time_ori = u_(6) - u_(5);
        double k = (-3+sqrt(9-4*(2-6*ratio)))/2;
        double time_new = k * time_ori;
        double delta_t = time_new - time_ori;

        cout << "i=2" << " time_ori=" << time_ori << " k=" << k << " time_new=" << time_new << " delta_t=" << delta_t << endl;
      ...
      cout << "i=" << i << endl;
      cout.precision(3);
      cout << u_.transpose() << endl;
      cout << (u_.block(1,0,u_.rows()-1,1) - u_.block(0,0,u_.rows()-1,1)).transpose() << endl;
...
bool NonUniformBspline::reallocateTime_mod2(bool show) {
  // SETY << "[Bspline]: total points size: " << control_points_.rows() << endl;
  // cout << "origin knots:\n" << u_.transpose() << endl;
  bool fea = true;
  ...
  // double u3u4 = u_(4)-u_(3);
  // double u4u5 = u_(5)-u_(4);
  // double u5u6 = u_(6)-u_(5);

  /* check vel feasibility and insert points */
  for (int i = 2; i < P.rows() - 1; ++i) {
    Eigen::VectorXd vel = p_ * (P.row(i + 1) - P.row(i)) / (u_(i + p_ + 1) - u_(i + 1));
  ...
  // {
  //   iter = 0;
  //   double u3u4_ratio = (u_(4)-u_(3))/interval_;
  //   double u4u5_ratio = (u_(5)-u_(4))/interval_;
  //   double u5u6_ratio = (u_(6)-u_(5))/interval_;

  //   cout << "u3u4_ratio=" << u3u4_ratio << endl;
  //   cout << "u4u5_ratio=" << u4u5_ratio << endl;
  //   cout << "u5u6_ratio=" << u5u6_ratio << endl;

  //   cout << "u_(2)=" << u_(2) << " u_(1)=" << u_(1) << " u_(0)=" <<  u_(0) << endl;

  //   double change = (u_(2)-u_(3))/u3u4_ratio - (u_(2)-u_(3));
  //   u_(2) = (u_(2)-u_(3))/u3u4_ratio + u_(3);
  //   u_(1) += change;
  //   u_(0) += change;

  //   cout << "u_(2)=" << u_(2) << " u_(1)=" << u_(1) << " u_(0)=" <<  u_(0) << endl;

  //   change = (u_(1)-u_(2))/u4u5_ratio - (u_(1)-u_(2));
  //   u_(1) = (u_(1)-u_(2))/u4u5_ratio + u_(2);
  //   u_(0) += change;

  //   cout << "u_(2)=" << u_(2) << " u_(1)=" << u_(1) << " u_(0)=" <<  u_(0) << endl;

  //   u_(0) = (u_(0)-u_(1))/u5u6_ratio + u_(1);
...
bool NonUniformBspline::adjustFirst3CtrlPoints(const Eigen::Vector3d pos, const Eigen::Vector3d vel, const Eigen::Vector3d acc)
{

  //cout << "u_=" << endl <<  u_ << endl;

  double interval_max = -1.0;
  for ( int i=0; i<6; i++ )
  {
    if ( interval_max < u_(i+1)-u_(i) )
      interval_max = u_(i+1)-u_(i);
  }
  ...
  double inc = 3*interval_max - u_(6);
  u_(4) = interval_max;
  u_(5) = 2*interval_max;
  u_(6) = 3*interval_max;

  for ( int i=7; i<u_.rows(); i++ )
    u_(i) += inc;
  ...
  A << 1, 4, 1, -1, 0, 1, 1, -2, 1;
  A.row(0) *= 1/6.0;
  A.row(1) *= (1 / 2.0 / interval_max);
  A.row(2) *= (1 / interval_max / interval_max);

  // write B
  Eigen::Matrix3d B;
  B.row(0) = pos.transpose();
  B.row(1) = vel.transpose();
  B.row(2) = acc.transpose();

  // cout << "A=" << endl << A << endl;
  // cout << "B=" << endl << B << endl;

  // solve Ax = B
  Eigen::Matrix3d ctrl_pts;
  for ( int i=0; i<3; i++ )
    ctrl_pts.col(i) = A.colPivHouseholderQr().solve(B.col(i));

  //cout << "control_points_=" << endl << control_points_.block(0,0,3,3) << endl << endl;;

  control_points_.block(0,0,3,3) = ctrl_pts;

  //cout << "ctrl_pts=" << endl << ctrl_pts << endl << endl;;
}
