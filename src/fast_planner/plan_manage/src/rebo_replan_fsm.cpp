...
void ReboReplanFSM::init(ros::NodeHandle& nh) {
  ...
  data_disp_pub_ = nh.advertise<plan_manage::DataDisp>("/planning/data_display", 100);
}
...
void ReboReplanFSM::waypointCallback(const nav_msgs::PathConstPtr& msg) {
  ...
  if ( success )
  {
    auto info = &planner_manager_->global_data_;


    // ROS_WARN("AAAAAAAAAAAAAAAAAAAAAAAAAAA");
    // // cout << "global_duration_=" << info->global_duration_ << endl;
    // // for ( double t=0; t<info->global_duration_; t+=0.1 )
    // //   cout << info->global_traj_.evaluate(t).transpose() << endl;
    // vector<vector<double>> x,y,z;
    // x = info->global_traj_.getCoef(0); 
    // y = info->global_traj_.getCoef(1); 
    // z = info->global_traj_.getCoef(2); 
    // for ( auto x_block:x )
    // {
    //   for ( auto x_ele:x_block )
    //     cout << x_ele << " ";
    //   cout << endl;
    // }
    // for ( auto y_block:y )
    // {
    //   for ( auto y_ele:y_block )
    //     cout << y_ele << " ";
    //   cout << endl;
    // }
    // for ( auto z_block:z )
    // {
    //   for ( auto z_ele:z_block )
    //     cout << z_ele << " ";
    //   cout << endl;
    // }
    ...
    for ( double t=0; t<planner_manager_->global_data_.global_duration_; t+=0.1 )
      gloabl_traj.push_back( planner_manager_->global_data_.global_traj_.evaluate(t) );

    visualization_->displayInitList(gloabl_traj, 1234);
...
void ReboReplanFSM::changeFSMExecState(FSM_EXEC_STATE new_state, string pos_call) {
  ...
  string state_str[7] = { "INIT", "WAIT_TARGET", "GEN_NEW_TRAJ", "REPLAN_TRAJ", "EXEC_TRAJ", "REPLAN_NEW", "EMERGENCY_STOP" };
  ...
  switch (exec_state_) {
    ...
    case GEN_NEW_TRAJ: {
      ...
      if (success) {
        ...
        // if (checkCollision()) cout << "\033[31mZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ\033[0m" << endl;
        // else cout << "\033[31mXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\033[0m" << endl;
    ...
    case REPLAN_TRAJ: {
      ...
      else 
      {
        ...
        else
        {
          ...
          {
            // Collision checker will handle this.
            //changeFSMExecState(GEN_NEW_TRAJ, "FSM");
          }
        }
        //changeFSMExecState(REPLAN_TRAJ, "FSM");
...
void ReboReplanFSM::checkCollisionCallback(const ros::TimerEvent& e) {
  ...
  // if ( map->getInflateOccupancy( odom_pos_ ) )
  // {
  //   ROS_ERROR("Fuck! The drone is in obstacles now!");
  //   callEmergencyStop( odom_pos_ );
  ...
  if (exec_state_ == FSM_EXEC_STATE::EXEC_TRAJ) {
    ...
    for ( double t=t_cur; t<info->duration_; t+=time_step )
    {
      ...
      {
        if ( t-t_cur < 0.5 ) // 0.3s of emergency time
        {
...
bool ReboReplanFSM::callReboundReplan(bool flag_use_poly_init, bool flag_randomPolyTraj) {
  ...
  /*** data display ***/
  data_disp_.a++;
  if ( data_disp_.a > 50 ) data_disp_.a = 50;
  data_disp_.b = start_vel_(0);
  data_disp_.c = start_acc_(0);
